...
**Fuente de los Datos:** Los datos son simulados y representan ventas de un periodo anual con sus repectivos puntos de entregas georeferenciados. Se dispone de:
...
**Fuente de los Datos:** Los datos provienen de registros históricos, representativos de un escenario real.
...
-   Se verificaron valores nulos en todos los campos del data set. Comenzamos por eliminar las lineas con IdCliente nulo (consideramos que no es representativo y puede afectar el analisis)
...
-   Antes de continuar, consideramos necesesario saber que porcentaje de ventas representan las lineas sin el dato de latitud y longitud:
...
El porcentaje de ventas sin el dato de georeferencia representa el 37%. Consideramos que hacer el analisis sobre el un porcentaje mayor al 60% es representativo, por lo que procedemos a eliminar las lineas sin ese dato.
...
-   **Latitud y Logitud:** Asegurar que sean de tipo numérico.
...
En la revision de valores extremos podemos ver que hay datos mal cargados: En la variable Latitud hay valores de longitud y viceversa
...
### Distribución de los Volúmenes de Ventas

A continuación, se presenta un gráfico que muestra la distribución de los volúmenes de ventas. Este gráfico permite identificar concentraciones de clientes y volúmenes atípicos.
...
# Agrupar datos por Latitud y Longitud y calcular el volumen total
Df_cluster_volumen <- Df %>%
  group_by(Latitud, Logitud) %>%
  summarise(
    Volumen_Total = sum(Cantidad, na.rm = TRUE),
    etiqueta = paste0(
      "Ubicación:<br>",
      "<b>Latitud:</b> ", Latitud, "<br>",
      "<b>Longitud:</b> ", Logitud, "<br>",
      "<b>Volumen Total:</b> ", format(Volumen_Total, big.mark = ".", decimal.mark = ",", scientific = FALSE), " kg"
    ),
    .groups = "drop" # Desagrupa después de consolidar
  )

# Crear el mapa por clúster de volúmenes
leaflet(Df_cluster_volumen) %>%
  addProviderTiles("OpenStreetMap.Mapnik") %>%
  addCircleMarkers(
    lat = ~Latitud,
    lng = ~Logitud,
    radius = ~sqrt(Volumen_Total) / 5, # Ajusta el radio proporcional al volumen
    color = "blue",
...
# Calcular el volumen total vendido por tipo de artículo
Df_volumen <- Df %>%
  group_by(TipoArticulo) %>%
  summarise(Volumen_Total = sum(Cantidad, na.rm = TRUE)) %>%
  arrange(desc(Volumen_Total)) %>%
...
# Crear etiquetas informativas para los puntos
Df <- Df %>%
  mutate(
    etiqueta = paste(
      "Cliente:", IdCliente,
...
for (i in 1:nrow(Df_volumen)) {
  ...
  mapa_todos_tipos <- mapa_todos_tipos %>%
    addCircleMarkers(
      data = Df %>% filter(TipoArticulo == tipo),
      lat = ~Latitud,
      lng = ~Logitud,
...
En esta sección se busca determinar la ubicación más óptima para construir una nueva planta productiva, utilizando un análisis basado en el volumen de ventas y la ubicación geográfica de los clientes.
...
Para encontrar la ubicación más óptima, se implementó un **método del Centro de Gravedad**. Este método permite calcular una ubicación central ponderada, teniendo en cuenta las ventas de cada cliente. En otras palabras, cada cliente "tira" hacia la ubicación de la planta con una fuerza proporcional a su volumen de ventas.
...
A continuación, se presenta el resultado del análisis, mostrando la ubicación óptima calculada para la nueva planta productiva. Esta ubicación se representa como el **Centro de Gravedad**, calculado utilizando los datos de ventas y ubicaciones de los clientes.
...
leaflet(Df) %>%
  ...
  addMarkers(
    ...
    icon = icons(
      iconUrl = "planta_icono.png",  # Aquí se usa el ícono personalizado
...
leaflet(Df_PRE) %>%
  ...
  addMarkers(
    ...
    icon = icons(
      iconUrl = "planta_icono.png",  # Asegúrate de que este archivo esté disponible
      iconWidth = 30,
      iconHeight = 30
    ),
...
Resultados para los tipos de articulos mas importantes:

-Productos PRE: Localidad mas cercana Chilibroste, Córdoba. -Productos CON: Localidad mas cercana Arias, Córdoba. -Productos SIL: Localidad mas cercana Oliva, Córdoba.
